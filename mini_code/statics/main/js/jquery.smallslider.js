...
(function ($) {
    ...
    $.smallslider = function (elm, options) {
        ...
        // 初始化对象
        _this.init = function () {
            _this.$ul = _this.$elm.find('>ul:first'); // 为子元素 ul
            ...
            if (_this.itemNums > 1) {
                ...
                if (_this.opts.showNextbtns){
                    _this.createUpNextbtns(); // 创建向左向右按钮
                }
                if (_this.opts.bindButtons){
                    _this.bindBtns();         // 绑定按钮
                }
                ...
                if (_this.opts.onBtnStop) {
                    _this.onBtns();
                }

            }
        };
        _this.init_bindBtnF=function(){
            _this.$ul = _this.$elm.find('>ul:first'); // 为子元素 ul
            _this.$lis = _this.$ul.find('li'); // 为所有 ul 下子元素 li 数组
            _this.$ims = _this.$ul.find('img'); // 为所有 li 下子元素 img 数组
            _this.itemNums = _this.$lis.length;
            _this.width =  _this.$ul.width();
            _this.height =  _this.$ul.height();
            _this.current = 0; // 当前的 index 索引
        ...
        //绑定切换按钮，不自动创建
        _this.bindBtns=function(){

            _this.$btns = $(_this.opts.btnSelector +" li");

            _this.$btns.removeClass(_this.opts.btnSelectClass);
            _this.$btns.eq(0).addClass(_this.opts.btnSelectClass);
            _this.opts.bindButtonsStyle?_this.$btns.eq(0).css("top","-6px"):""; //添加top值
            if (_this.opts.switchMode == 'click') {
                _this.$btns.click(function () {
                    var ix = _this.$btns.index($(this));
                    _this.slideTo(ix); // 表示需要切换到哪一张
                ...
                _this.$btns.hover(function () {
                    var ix = _this.$btns.index($(this));
                    _this.slideTo(ix);
                    if( _this.opts.bindButtonsStyle){//如果是要求悬浮那么修改top
                        _this.$btns.each(function(){$(this).stop().animate({top:"0px"},300);});    //添加top值
                        $(this).stop().animate({top:"-6px"},300);
                    }
                });
            }
        };
        //创建向左向右 按钮
        _this.createUpNextbtns=function(){
            var l,r;
            l=_this.$elm.find("a[left]");
            r=_this.$elm.find("a[right]");
            l.click(function(){
        ...
        _this.slideTo = function (index) {
            ...
            if (_this.opts.showButtons||_this.opts.bindButtons) {
                _this.$btns.removeClass(_this.opts.btnSelectClass?_this.opts.btnSelectClass:'current-btn');
                _this.$btns.eq(index).addClass(_this.opts.btnSelectClass?_this.opts.btnSelectClass:'current-btn');
                //添加top值
                if(_this.opts.bindButtonsStyle){
                    _this.$btns.each(function(){$(this).stop().animate({top:"0px"},300);});    //添加top值
                    _this.$btns.eq(index).stop().animate({top:"-6px"},300);
                }
            }
    ...
    $.smallslider.defaults = {
        ...
        onBtnStop: false, // 鼠标放在按钮上时，是否停止播放
        ...
        showNextbtns:false,//是否显示向左向右btn
        bindButtons:false,//默认不绑定
        bindButtonsStyle:false,//btnStyle 是否向上浮动
        btnSelector:"",   //按钮selector
        btnSelectClass:"", //按钮 选中样式
