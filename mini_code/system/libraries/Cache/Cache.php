...
class CI_Cache extends CI_Driver_Library {
	...
	{	
		if ($this->_adapter == 'memcached' || $this->_adapter == 'memcache' ){
			return $this->{$this->_adapter}->get($id,$is_valid);
		}
	 ...
	 * @update author :  jackcao
	 $ @update desc : memcachedb   没有过期时间 ，应用层中增加 过期时间 ，方便 定时操作
	 $ @update $is_expire :  true  为  没有过期时间，false 为 有过期时间 
     :wa
	 */
	public function save($id, $data, $ttl = 60, $is_expire = true)
	{

		if (!$is_expire){
			return $this->{$this->_adapter}->saveExpire($id, $data, $ttl, $is_expire);
		}
